...
function createFiles(options) {
    ...
    return schematics_1.mergeWith(schematics_1.apply(schematics_1.url(`./files/lib`), [
        schematics_1.template(Object.assign(Object.assign({}, options), { className,
            name,
            propertyName, cliCommand: 'nx', tmpl: '', offsetFromRoot: devkit_1.offsetFromRoot(options.projectRoot), hasUnitTestRunner: options.unitTestRunner !== 'none' })),
...
exports.libraryGenerator = ngcli_adapter_1.wrapAngularDevkitSchematic('@nrwl/workspace', 'library');
...
function addTestFiles(options) {
    return options.unitTestRunner === 'none'
        ? schematics_1.filter((path) => !(path.endsWith('spec.ts') || path.endsWith('spec.tsx')))
        : schematics_1.noop();
}
