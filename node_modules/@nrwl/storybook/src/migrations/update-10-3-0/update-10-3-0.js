...
function default_1(tree, context) {
    return schematics_1.chain([update, workspace_1.formatFiles()]);
}
exports.default = default_1;
function update(tree, context) {
    return workspace_1.updateWorkspaceInTree((config, context, tree) => {
...
function updateLintTarget(tree, context, options) {
    ...
    const paths = {
        ...
        tsConfigStorybook: core_1.normalize(path.join(options.projectConfig.root, '.storybook/tsconfig.json')),
    };
    ...
    const isReactProject = utils_1.isFramework('react', {
        uiFramework: (_a = architect.storybook.options) === null || _a === void 0 ? void 0 : _a.uiFramework,
    });
    const mainTsConfigContent = utils_1.getTsConfigContent(tree, paths.tsConfig);
    const tsConfig = {
        main: mainTsConfigContent,
        lib: tree.exists(paths.tsConfigLib)
            ? utils_1.getTsConfigContent(tree, paths.tsConfigLib)
            : mainTsConfigContent,
        storybook: utils_1.getTsConfigContent(tree, paths.tsConfigStorybook),
    };
    if (isReactProject && Array.isArray(tsConfig.lib.exclude)) {
        ...
        tree.overwrite(paths.tsConfigLib, workspace_1.serializeJson(tsConfig.lib));
    }
    if (Array.isArray(tsConfig.main.references) &&
        ...
        tsConfig.main.references.push({
            path: core_1.normalize('./.storybook/tsconfig.json'),
        });
        tree.overwrite(paths.tsConfig, workspace_1.serializeJson(tsConfig.main));
    }
    if (isReactProject && Array.isArray(tsConfig.storybook.exclude)) {
        ...
        tree.overwrite(paths.tsConfigStorybook, workspace_1.serializeJson(tsConfig.storybook));
    }
    // update workspace
    if (architect.lint &&
